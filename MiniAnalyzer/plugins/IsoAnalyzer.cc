...
class IsoAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   ...
   private:
      ...
      float b_pt, b_eta, b_phi, b_energy;
      int b_chn, b_nhn, b_phn;
      float b_ch_dR, b_ch_dPhi, b_ch_dEta;
      float b_nh_dR, b_nh_dPhi, b_nh_dEta;
      float b_ph_dR, b_ph_dPhi, b_ph_dEta;

      float b_chiso0_005, b_chiso005_01, b_chiso01_015, b_chiso015_02, b_chiso02_025, b_chiso025_03;
      float b_nhiso0_005, b_nhiso005_01, b_nhiso01_015, b_nhiso015_02, b_nhiso02_025, b_nhiso025_03;
      float b_phiso0_005, b_phiso005_01, b_phiso01_015, b_phiso015_02, b_phiso02_025, b_phiso025_03;
      float b_puiso0_005, b_puiso005_01, b_puiso01_015, b_puiso015_02, b_puiso02_025, b_puiso025_03;
      float b_chiso, b_schiso, b_nhiso, b_phiso, b_puiso;
      float b_absiso, b_reliso, b_neutral;
      float b_chiso_org, b_puChiso_org, b_nhiso_org, b_phiso_org, b_absiso_org, b_reliso_org, b_neutral_org;

      float b_chiso03_org, b_puChiso03_org, b_nhiso03_org, b_phiso03_org, b_absiso03_org, b_reliso03_org, b_neutral03_org;

      float b_trackiso, b_ecaliso, b_hcaliso;

      int check_sb;
//for cnn variables
      std::vector<double>* b_ch_Phi;
      std::vector<double>* b_ch_Eta;
      std::vector<double>* b_ch_E;
      std::vector<double>* b_ch_pt;
      std::vector<double>* b_vch_dPhi;
      std::vector<double>* b_vch_dEta;

      std::vector<double>* b_nh_Phi;
      std::vector<double>* b_nh_Eta;
      std::vector<double>* b_nh_E;
      std::vector<double>* b_nh_pt;
      std::vector<double>* b_vnh_dPhi;
      std::vector<double>* b_vnh_dEta;

      std::vector<double>* b_ph_Phi;
      std::vector<double>* b_ph_Eta;
      std::vector<double>* b_ph_E;
      std::vector<double>* b_ph_pt;
      std::vector<double>* b_vph_dPhi;
      std::vector<double>* b_vph_dEta;
...
//for cnn variables
  b_ch_Phi = new std::vector<double>();
  b_ch_Eta = new std::vector<double>();
  b_ch_E = new std::vector<double>();
  b_ch_pt = new std::vector<double>();
  b_vch_dPhi = new std::vector<double>();
  b_vch_dEta = new std::vector<double>();
  
  b_nh_Phi = new std::vector<double>();
  b_nh_Eta = new std::vector<double>();
  b_nh_E = new std::vector<double>();
  b_nh_pt = new std::vector<double>();
  b_vnh_dPhi = new std::vector<double>();
  b_vnh_dEta = new std::vector<double>();

  b_ph_Phi = new std::vector<double>();
  b_ph_Eta = new std::vector<double>();
  b_ph_E = new std::vector<double>();
  b_ph_pt = new std::vector<double>();
  b_vph_dPhi = new std::vector<double>();
  b_vph_dEta = new std::vector<double>();
...
//for cnn variables
    ...
    for (unsigned int i = 0, n = pfcands->size(); i < n; ++i) {
      ...
      float dR = deltaR( muon.p4(), pfc.p4() );
      float dPhi = deltaPhi( muon.phi(), pfc.phi() );
      float dEta = muon.eta() - pfc.eta();
      float iso = pfc.pt();
      ...
      if( absId ==  211 ) {  // charged hadron
        if( dR < 0.3 ) {
          ...
          if( pfc.fromPV() >= 2 ){ 
            ...
            b_ch_Phi->push_back(ch_phi);
            b_ch_Eta->push_back(ch_eta);
            b_ch_E->push_back(ch_e);
            b_ch_pt->push_back(ch_pt);

            vch_deta = vch_deta + dEta; 
            vch_dphi = vch_dphi + dPhi;
            b_vch_dPhi->push_back(vch_dphi);
            b_vch_dEta->push_back(vch_deta);
      ...
      if( absId ==  130 ) {
        if( dR < 0.3 ) {
          ...
          b_nh_Phi->push_back(nh_phi);
          b_nh_Eta->push_back(nh_eta);
          b_nh_E->push_back(nh_e);
          b_nh_pt->push_back(nh_pt);

          vnh_deta = vnh_deta + dEta; 
          vnh_dphi = vnh_dphi + dPhi;
          b_vnh_dPhi->push_back(vnh_dphi);
          b_vnh_dEta->push_back(vnh_deta);
      ...
      if( absId == 22 ) {
        if( dR < 0.3 ) {
          ...
          b_ph_Phi->push_back(ph_phi);
          b_ph_Eta->push_back(ph_eta);
          b_ph_E->push_back(ph_e);
          b_ph_pt->push_back(ph_pt);

          vph_deta = vph_deta + dEta; 
          vph_dphi = vph_dphi + dPhi;
          b_vph_dPhi->push_back(vph_dphi);
          b_vph_dEta->push_back(vph_deta);
...
void IsoAnalyzer::beginJob(){
   ...
   tree->Branch("energy" , &b_energy , "energy/f");
   tree->Branch("pt" , &b_pt , "pt/f");
   tree->Branch("eta", &b_eta, "eta/f");
   tree->Branch("phi", &b_phi, "phi/f");

   tree->Branch("chIso0_005" , &b_chiso0_005 , "chiso0_005/f");
   tree->Branch("chIso005_01", &b_chiso005_01, "chiso005_01/f");
   tree->Branch("chIso01_015", &b_chiso01_015, "chiso01_015/f");
   tree->Branch("chIso015_02", &b_chiso015_02, "chiso015_02/f");
   tree->Branch("chIso02_025", &b_chiso02_025, "chiso02_025/f");
   tree->Branch("chIso025_03", &b_chiso025_03, "chiso025_03/f");
   tree->Branch("chIso"      , &b_chiso      , "chiso/f");
   tree->Branch("schIso"     , &b_schiso     , "schiso/f");

   tree->Branch("nhIso0_005" , &b_nhiso0_005 , "nhiso0_005/f");
   tree->Branch("nhIso005_01", &b_nhiso005_01, "nhiso005_01/f");
   tree->Branch("nhIso01_015", &b_nhiso01_015, "nhiso01_015/f");
   tree->Branch("nhIso015_02", &b_nhiso015_02, "nhiso015_02/f");
   tree->Branch("nhIso02_025", &b_nhiso02_025, "nhiso02_025/f");
   tree->Branch("nhIso025_03", &b_nhiso025_03, "nhiso025_03/f");
   tree->Branch("nhIso"      , &b_nhiso      , "nhiso/f");

   tree->Branch("phIso0_005" , &b_phiso0_005 , "phiso0_005/f");
   tree->Branch("phIso005_01", &b_phiso005_01, "phiso005_01/f");
   tree->Branch("phIso01_015", &b_phiso01_015, "phiso01_015/f");
   tree->Branch("phIso015_02", &b_phiso015_02, "phiso015_02/f");
   tree->Branch("phIso02_025", &b_phiso02_025, "phiso02_025/f");
   tree->Branch("phIso025_03", &b_phiso025_03, "phiso025_03/f");
   tree->Branch("phIso"      , &b_phiso      , "phiso/f");

   tree->Branch("puiso0_005" , &b_puiso0_005 , "puiso0_005/f");
   tree->Branch("puiso005_01", &b_puiso005_01, "puiso005_01/f");
   tree->Branch("puiso01_015", &b_puiso01_015, "puiso01_015/f");
   tree->Branch("puiso015_02", &b_puiso015_02, "puiso015_02/f");
   tree->Branch("puiso02_025", &b_puiso02_025, "puiso02_025/f");
   tree->Branch("puiso025_03", &b_puiso025_03, "puiso025_03/f");
   tree->Branch("puiso"      , &b_puiso      , "puiso/f");

   tree->Branch("abIso" , &b_absiso, "absiso/f");
   tree->Branch("relIso", &b_reliso, "reliso/f");

   tree->Branch("chIso_org"  , &b_chiso_org  , "chiso_org/f");
   tree->Branch("puChIso_org", &b_puChiso_org, "puChiso_org/f");
   tree->Branch("nhIso_org"  , &b_nhiso_org  , "nhiso_org/f");
   tree->Branch("phIso_org"  , &b_phiso_org  , "phiso_org/f");
   tree->Branch("absIso_org" , &b_absiso_org , "absiso_org/f");
   tree->Branch("relIso_org" , &b_reliso_org , "reliso_org/f");

   tree->Branch("chIso03_org"  , &b_chiso03_org  , "chiso03_org/f");
   tree->Branch("puChIso03_org", &b_puChiso03_org, "puChiso03_org/f");
   tree->Branch("nhIso03_org"  , &b_nhiso03_org  , "nhiso03_org/f");
   tree->Branch("phIso03_org"  , &b_phiso03_org  , "phiso03_org/f");
   tree->Branch("absIso03_org" , &b_absiso03_org , "absiso03_org/f");
   tree->Branch("relIso03_org" , &b_reliso03_org , "reliso03_org/f");

   tree->Branch("chN", &b_chn, "chn/I");
   tree->Branch("nhN", &b_nhn, "nhn/I");
   tree->Branch("phN", &b_phn, "phn/I");

   tree->Branch("ch_dR", &b_ch_dR, "ch_dR/f");
   tree->Branch("nh_dR", &b_nh_dR, "nh_dR/f");
   tree->Branch("ph_dR", &b_ph_dR, "ph_dR/f");

   tree->Branch("ch_dPhi", &b_ch_dPhi,"ch_dPhi/f");
   tree->Branch("nh_dPhi", &b_nh_dPhi,"nh_dPhi/f");
   tree->Branch("ph_dPhi", &b_ph_dPhi,"ph_dPhi/f");

   tree->Branch("ch_dEta", &b_ch_dEta, "ch_dEta/f");
   tree->Branch("nh_dEta", &b_nh_dEta, "nh_dEta/f");
   tree->Branch("ph_dEta", &b_ph_dEta, "ph_dEta/f");

   tree->Branch("trackIso", &b_trackiso, "trackIso/f");
   tree->Branch("ecalIso", &b_ecaliso, "ecalIso/f");
   tree->Branch("hcalIso", &b_hcaliso, "hcalIso/f");

   tree->Branch("check_sb", &check_sb, "check_sb/i");

/*   tree->Branch("ch_Phi", "std::vector<double>", &b_ch_Phi);
   tree->Branch("ch_Eta", "std::vector<double>", &b_ch_Eta);
   tree->Branch("ch_E", "std::vector<double>", &b_ch_E);
   tree->Branch("ch_pt", "std::vector<double>", &b_ch_pt);
   tree->Branch("vch_dPhi", "std::vector<double>", &b_vch_dPhi);
   tree->Branch("vch_dEta", "std::vector<double>", &b_vch_dEta);

   tree->Branch("nh_Phi", "std::vector<double>", &b_nh_Phi);
   tree->Branch("nh_Eta", "std::vector<double>", &b_nh_Eta);
   tree->Branch("nh_E", "std::vector<double>", &b_nh_E);
   tree->Branch("nh_pt", "std::vector<double>", &b_nh_pt);
   tree->Branch("vnh_dPhi", "std::vector<double>", &b_vnh_dPhi);
   tree->Branch("vnh_dEta", "std::vector<double>", &b_vnh_dEta);

   tree->Branch("ph_Phi", "std::vector<double>", &b_ph_Phi);
   tree->Branch("ph_Eta", "std::vector<double>", &b_ph_Eta);
   tree->Branch("ph_E", "std::vector<double>", &b_ph_E);
   tree->Branch("ph_pt", "std::vector<double>", &b_ph_pt);
   tree->Branch("vph_dPhi", "std::vector<double>", &b_vph_dPhi);
   tree->Branch("vph_dEta", "std::vector<double>", &b_vph_dEta);
*/
}
